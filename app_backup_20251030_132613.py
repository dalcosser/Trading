...
def _load_daily_df(ticker: str) -> pd.DataFrame | None:
    ...
    try:
        ...
        if not dcol:
            return None
        out = pd.DataFrame({'Date': pd.to_datetime(dfp[dcol], errors='coerce')})
        for name in ('Open','High','Low','Close','Volume'):
            c = cols.get(name.lower())
            if c:
                out[name] = pd.to_numeric(dfp[c], errors='coerce')
...
# Removed fragile path auto-discovery and sidebar overrides to ensure fast, reliable startup
    ...
    if scan_type == 'Single Ticker':
        ...
        if run_single:
            ...
            if sdf is None or sdf.empty or (need_open and 'Open' not in sdf.columns):
                ...
                elif strategy.startswith('Golden/Death'):
                    ev = _ma_cross_events(sdf, 50, 200)
                elif strategy.startswith('Inside/Outside'):
                    ev = _inside_outside_events(sdf)
                elif strategy.startswith('Volume Spike'):
                    ev = _volume_spike_events(sdf, 2.0, 20)
                elif strategy.startswith('RS 52w High'):
                    spy = _load_parquet_daily('SPY') or pd.DataFrame()
                    if spy.empty:
                        st.error('SPY data not found for RS scan.')
                        ev = pd.DataFrame()
                    else:
